...
class NRF93
{
	private $energia, $proteinas, $fibras, $vit_A, $vit_C, $vit_E, $calcio, $hierro, $magnesio, $potasio, $sodio, $grasas_tot, $azucares;

	private $vr_energia, $vr_proteinas, $vr_fibras, $vr_vit_A, $vr_vit_C, $vr_vit_E, $vr_calcio, $vr_hierro, $vr_magnesio, $vr_potasio, $vr_sodio, $vr_grasas_tot, $vr_azucares;	
	...
	public function getNRF9(){
		return 100*( ($this->fibras/$this->vr_fibras) + ($this->proteinas/$this->vr_proteinas) + ($this->vit_A/$this->vr_vit_A) + ($this->vit_C/$this->vr_vit_C) + ($this->vit_E/$this->vr_vit_E) + ($this->calcio/$this->vr_calcio) + ($this->hierro/$this->vr_hierro) + ($this->magnesio/$this->vr_magnesio) + ($this->potasio/$this->vr_potasio) );
	}

	public function getNRF3(){
		return 100*( ($this->grasas_tot/$this->vr_grasas_tot) + ($this->sodio/$this->vr_sodio) + ($this->azucares/$this->vr_azucares) );
	}
